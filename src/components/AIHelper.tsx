  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-[calc(100vh-140px)] min-h-[550px]">
      ...
      <div className="lg:col-span-4 bg-[#fbfaf6] border border-amber-200/50 rounded-xl p-4 flex flex-col justify-between space-y-4">
            ...
            <div className="flex-1 flex flex-col overflow-hidden text-xs space-y-3">
              {isAddingDoc ? (
                ...
                <form onSubmit={handleAddDocument} className="bg-white border border-stone-150 rounded-lg p-3 space-y-3 overflow-y-auto max-h-[350px]">
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-stone-650 block">Nội dung chi văn tế phả:*</label>
                    <textarea 
                      rows={5}
                      required
                      placeholder="Mời chép nội dung sắc phong, thế trạch, mốc lịch sử..."
                      value={docContent}
        ...
        {/* Informative advice */}
        <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded text-[11px] text-stone-600 leading-relaxed shrink-0">
          💡 **Lời Trị Sự**: Trợ lý AI tự học dựa sát gia sử mộc bản và tài liệu lưu trữ ở trên để lập sớ tế nghiêm khắc chính gốc, không bịa sớ rác.
        </div>
      </div>0.5 shrink-0" />
                <div>
                  <p className="font-bold text-stone-800 leading-tight">Tra cứu Sử tịch bản họ tự sự</p>
                  <span className="text-[10px] text-stone-400 font-medium block leading-normal mt-0.5">Xuất bối thế thứ sơ khởi ở Hoa Lư xưa</span>
                </div>
              </div>
            </button>
          </div>
        ...
        {/* Informative advice */}
        <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded text-[11px] text-stone-600 leading-relaxed">
          💡 **Gợi ý của Trị Sự**: Sau khi AI sản sinh hoàn thành bài văn tế hoặc bản dịch thư ngỏ, Quý nhân hãy sao chép ra máy tính và biên tập ấn loát, đóng mộc triện đỏ dòng họ để hành lễ dâng hương bái niệm tôn trọng.
        </div>
      </div>
